...
EMAIL_QUEUE_FILE = Path(os.getenv("EMAIL_CADENCE_QUEUE_FILE", "/root/sdr-vps/data/email_cadence_queue.json"))
...
def load_email_queue_rows():
    try:
        import json
        if not EMAIL_QUEUE_FILE.exists():
            return None
        payload = json.loads(EMAIL_QUEUE_FILE.read_text(encoding="utf-8-sig") or "[]")
        return payload if isinstance(payload, list) else []
    except Exception as exc:
        print(f"[WA_HUNTER_EMAIL_QUEUE_READ_FAIL] path={EMAIL_QUEUE_FILE} erro={exc}")
        return []


def deal_field_text(deal, *names):
    values = []
    for name in names:
        value = (deal or {}).get(name)
        if value is not None:
            values.append(str(value))
    return " ".join(values).strip().lower()


def fallback_allows_hunter(deal, tokens):
    deal_id = int((deal or {}).get("id") or 0)
    stage_name = deal_stage_name(deal)
    status_text = deal_field_text(
        deal,
        "cadence_status",
    ...
    raw_text = " ".join([stage_name, status_text, " ".join(sorted(tokens))]).lower()
    print(f"[EMAIL_QUEUE_MISSING_FALLBACK] deal_id={deal_id} stage={stage_name or '-'} status={status_text or '-'}")
    blocked_terms = ("nutricao", "nutrição", "em_cadencia", "cadencia_email", "email_step_", "step_1", "step_2", "step_3", "step_4", "step_5")
    if any(term in raw_text for term in blocked_terms):
        return False, "fallback_nurturing_signal"
    completed_terms = ("cad6", "step_6", "email_6", "cadencia_finalizada", "cadence_done", "completed", "finalizado", "done")
    if any(term in raw_text for term in completed_terms):
        return True, "fallback_completed_signal"
    return False, "fallback_no_clear_completion_signal"


def nurturing_allows_hunter(deal, tokens):
    deal_id = int((deal or {}).get("id") or 0)
    rows = load_email_queue_rows()
    if rows is None:
        return fallback_allows_hunter(deal, tokens)
    matched = [row for row in rows if int((row or {}).get("deal_id") or 0) == int(deal_id or 0)]
    if not matched:
        return False, "email_queue_missing"
    row = matched[-1]
    status = str(row.get("status") or "").strip().lower()
    if status in {"clicked_warm", "warm", "replied", "won", "lost", "opt_out", "wrong_contact", "stopped"}:
        return True, status
    if status in {"done", "completed", "finalizado"}:
        return True, status
    if int(row.get("last_sent_step_email") or 0) >= 6:
        return True, "step6_sent"
    return False, f"email_status_{status or 'unknown'}_step_{int(row.get('last_sent_step_email') or 0)}"
...
def hunter_skip_reason(deal, state, blocked):
    ...
    if is_warm_deal(deal, state, phone):
        return "warm"
    hunter_allowed, nurturing_reason = nurturing_allows_hunter(deal, tokens)
    if not hunter_allowed:
        print(f"[HUNTER_BLOCKED_BY_NURTURING] deal_id={deal_id} phone={phone} reason={nurturing_reason}")
        return "nurturing_in_progress"
