...
def update_person_missing_fields(person_id, email=None, phone=None):
    payload={}
    if email:
    ...
    if payload:
        r=pd("PUT",f"/persons/{person_id}",payload)
        print("[DEDUP_PERSON_UPDATE]",person_id,bool(r))

def find_open_deal_by_phone_or_email(phone=None,email=None):
    terms=[phone,email]
    for term in terms:
        if not term: 
            continue
        r=requests.get(f"{BASE}/persons/search",params={"api_token":TOKEN,"term":term},timeout=30)
        for it in (r.json().get("data") or {}).get("items") or []:
            person_id=it.get("item",{}).get("id")
            if not person_id:
                continue
            dr=requests.get(f"{BASE}/persons/{person_id}/deals",params={"api_token":TOKEN,"status":"open"},timeout=30)
            deals=dr.json().get("data") or []
            if deals:
                deals=sorted(deals,key=lambda x:int(x.get("id") or 0))
                return deals[0].get("id"), person_id
    return None, None


def normalize_phone_for_dedupe(phone):
    digits = "".join(ch for ch in str(phone or "") if ch.isdigit())
    if digits.startswith("55") and len(digits) >= 12:
        return digits[-11:]
    return digits[-11:]

def find_open_deal_by_phone(phone):
    phone = normalize_phone_for_dedupe(phone)
    if not phone:
        return None

    r = requests.get(
        f"{BASE}/persons/search",
        params={"api_token": TOKEN, "term": phone, "fields": "phone"},
        timeout=30,
    )
    if r.status_code >= 400:
        print("[DEDUP_PHONE_SEARCH_FAIL]", r.status_code, r.text[:120])
        return None

    for item in (r.json().get("data") or {}).get("items") or []:
        person_id = item.get("item", {}).get("id")
        if not person_id:
            continue

        dr = requests.get(
            f"{BASE}/persons/{person_id}/deals",
            params={"api_token": TOKEN, "status": "open"},
            timeout=30,
        )
...
def clean_person_name(raw):
    ...
    raw = re.sub(r"<br\s*/?>", "\n", raw, flags=re.I)
    raw = re.sub(r"<[^>]+>", " ", raw)
    name = re.split(r"\n|E-mail:|Email:|Telefone:|WhatsApp:|Empresa:|Mensagem:|Data:|Horário:|Horario:|URL", raw, flags=re.I)[0]
    name = re.sub(r"\s+", " ", name).strip()
    return name[:100] or "Lead sem nome"

def extract_by_label(raw, label):
    raw = raw or ""
    txt = re.sub(r"<br\s*/?>", "\n", raw, flags=re.I)
...
def clean_company_name(raw):
    ...
    emp = extract_by_label(raw, "Empresa")
    if emp:
        emp = re.split(r"Mensagem:|Data:|Horário:|Horario:|URL", emp, flags=re.I)[0]

    if not emp:
        emp = raw

    emp = re.split(r"\n|E-mail:|Email:|Telefone:|WhatsApp:|Mensagem:|Data:|Horário:|Horario:|URL", emp, flags=re.I)[0]
    emp = re.sub(r"\s+", " ", emp).strip(" -|")
    return emp[:100] or ""

def clean_deal_title(empresa, nome):
    base = (empresa or nome or "Lead LP").strip()
    return f"Lead LP - {base}"[:150]
...
def extract(text, sender):
    ...
    return {
        "nome": clean_person_name(nome or txt),
        "empresa": clean_company_name(empresa or txt),
...
def main():
    ...
    for mid in data[0].split()[-80:]:
        ...
        title=clean_deal_title(lead.get("empresa"), lead.get("nome"))

        existing_deal_id, existing_person_id = find_open_deal_by_phone_or_email(phone=phone, email=email)
        if existing_deal_id:
            print("[DEDUP_DEAL_EXISTENTE]", existing_deal_id, email, phone)
            if existing_person_id:
                update_person_missing_fields(existing_person_id, email=email, phone=phone)
