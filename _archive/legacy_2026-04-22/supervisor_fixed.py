...
class SDRSupervisor:
    ...
    def process_inbound_message(self, msg):
        ...
        try:
            if not has_outbound_history:
                ...
                self._add_to_blocklist(phone, reason="numero_errado")
            elif intent == "unknown_person":
                reply = str(self.pitch.get_closing_message("unknown_person") or "").strip()
                self._add_to_blocklist(phone, reason="nao_conhece")
            elif intent == "como_funciona":
                print("[INTENT_DETECTADA] como_funciona")
                mensagens = [
                    "Boa pergunta — vou te resumir direto 👇",
                    "A gente estrutura campanhas promocionais gamificadas (tipo roleta e raspadinha).",
                    "O foco é transformar ação de marketing em venda real + coleta de dados.",
                    "Cuidamos de tudo: estratégia, regulamento (100% seguro) e o desenvolvimento.",
                    "Pra você ter ideia, já tivemos campanha com +93% de vendas em regiões específicas.",
                    "Se fizer sentido, te mostro isso na prática em 5 minutos.",
                    "Qual horário funciona melhor?"
                ]
                for msg in mensagens:
                    self.whatsapp_service.send_message(phone, msg, deal_id=int((lead or {}).get("id") or 0))
                    import time; time.sleep(1.5)
                print("[RESPOSTA_EXPLICACAO] enviada")
                print("[AVANCO_FUNIL] tentativa_agendamento")
                print("[MSG_CURTA_FORMATADA] fluxo finalizado")
                return {"ok": True, "confirmed": True, "reason": "explicou_valor"}
        ...
        if int((lead or {}).get("id") or 0) > 0:
            ...
            if intent in {"sem_interesse", "hostil", "wrong_number", "unknown_person", "indicacao"}:
                try:
    ...
    def _deal_people(self, deal, primary_person_id, embedded_person):
        ...
        if int(primary_person_id or 0):
            try:
                merge_person(self.crm.get_person(int(primary_person_id)))
            except Exception as exc:
        ...
        for candidate in candidates:
            ...
            try:
                ...
                if not (isinstance(result, dict) and result.get("ok") is True and result.get("confirmed") is not False):
                    continue
                if str(result.get("reason") or "").strip().lower() == "not_in_crm":
        ...
        try:
            ...
            while self.pending_whatsapp_queue:
                ...
                for num in lead_phones:
                    ...
                    print(f"[DELAY_HUMANO] Pausa estrategica de {delay}s...")
                    time.sleep(delay)
