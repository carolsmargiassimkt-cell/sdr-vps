...
STATE_JSON = Path('/root/sdr-vps/runtime/enrich_342_existing_only_state.json')
RESULT_JSON = Path('/root/sdr-vps/runtime/enrich_342_existing_only_result.json')
...
def api_get_all_open_deals(token):
    ...
    while True:
        r = requests.get(
            ...
            params={'api_token': token, 'start': start, 'limit': limit, 'status': 'open'},
            timeout=60,
        )
        r.raise_for_status()
        payload = r.json()
...
def api_search_orgs(token, term):
    r = requests.get(
        ...
        params={'api_token': token, 'term': term, 'fields': 'name', 'limit': 10, 'exact_match': False},
        timeout=60,
    )
    if r.status_code >= 400:
        return []
...
def choose_existing_deal(open_p2_deals, names):
    keys = [norm_key(x) for x in names if x]
    if not keys:
        return []
    matches = []
    for d in open_p2_deals:
        title_key = norm_key(d.get('title'))
        org_key = norm_key(d.get('org_name') or '')
        if any(k and (k == title_key or k == org_key) for k in keys):
            matches.append(d)
    return matches
...
for row in rows:
    ...
    if cnpj and oportun_token:
        ...
        if sc < 400 and payload:
            ...
            names_to_match.extend([ext.get('company_name'), ext.get('trade_name')])

    deal_matches = choose_existing_deal(open_p2, names_to_match)

    if len(deal_matches) != 1:
        result['status'] = 'needs_review'
        result['reason'] = 'no_unique_existing_deal_match'
        result['candidate_deal_ids'] = [int(d.get('id') or 0) for d in deal_matches[:10]]
    ...
    if org_id and (ext.get('trade_name') or ext.get('company_name')):
    ...
    if person_id:
        ...
        if payload_person:
            ...
            if 200 <= sc < 300:
                ...
                print(f'[UPDATED_EXISTING] deal={deal_id} company={company}', flush=True)
            else:
                result['status'] = 'error'
        ...
        if not emails and not phones:
            ...
            if existing_person_id:
                ...
                if 200 <= sc < 300:
                    ...
                    print(f'[LINKED_EXISTING] deal={deal_id} company={company}', flush=True)
                else:
                    result['status'] = 'error'
                ...
                if 200 <= sc1 < 300 and created.get('id'):
                    ...
                    if 200 <= sc2 < 300:
                        ...
                        print(f'[CREATED_AND_LINKED] deal={deal_id} company={company}', flush=True)
                    else:
                        result['status'] = 'error'
