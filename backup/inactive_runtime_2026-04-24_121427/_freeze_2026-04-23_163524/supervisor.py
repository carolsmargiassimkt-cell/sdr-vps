...
WHATSAPP_PRE_SEND_DELAY_MIN_SEC = 5

WHATSAPP_PRE_SEND_DELAY_MAX_SEC = 15
...
class SDRSupervisor:
    ...
    def process_inbound_message(self, msg):
        ...
        if phone in self.blocklist or self._conversation_state_blocks_automation(conversation_state):
            print(f"[INBOUND_BLOQUEADO_SEM_RESPOSTA] telefone={phone}")
            return {"ok": True, "confirmed": True, "reason": "blocked_contact"}
        ...
        if intent in NEGATIVE_STOP_INTENTS:
            ...
            if deal_id > 0:
                ...
                try:
                    ...
                    pass
            return {"ok": True, "confirmed": True, "reason": "manual_stop", "intent": intent}
    ...
    def _send_email(self, lead):
        ...
        self._safe_pipedrive_call(
            "deal_add_tag:email",
            lambda: self.crm.add_tag("deal", deal_id, self._email_tag(lead)),
            fallback=False,
        )
    ...
    def _next_whatsapp_delay(self):
        ...
        base_delay = max(45, int((WHATSAPP_DISTRIBUTION_WINDOW_SEC / target_total) - human_avg_delay))
        jitter = max(15, int(base_delay * float(WHATSAPP_SEND_GAP_JITTER_RATIO or 0.25)))
        min_delay = max(45, base_delay - jitter)
        max_delay = max(min_delay + 15, base_delay + jitter)
    ...
    def run(self):
        ...
        try:
            ...
            if remaining_today <= 0:
                print(f"[LIMITE_DIARIO_ATINGIDO] enviados={sent_today} cap={MAX_DEALS_DIA}")
                self._process_pending_emails()
                return
            leads = []
            added_to_queue = 0
            ...
            while self.pending_whatsapp_queue:
                ...
                if cadence_step == 1:
                    try:
                        cad1_ok = bool(
                            self._safe_pipedrive_call(
                                "deal_add_tag:cad1",
                                lambda: self.crm.add_tag("deal", deal_id, "cad1"),
                                fallback=False,
                            )
                        )
                        print(
                            f"[CRM_TAG_RESULT] entity=deal deal={deal_id} tag=cad1 "
                            f"ok={1 if cad1_ok else 0} "
                            f"http_status={int(getattr(self.crm, 'last_http_status', 0) or 0)} "
                            f"endpoint={str(getattr(self.crm, 'last_http_endpoint', '') or '-')}"
                        )
                    except Exception:
                ...
                if cadence_step == 1:
                    print(
                        f"[CAD1_STAGE_SYNC] deal={deal_id} cadencia={stage_step} "
