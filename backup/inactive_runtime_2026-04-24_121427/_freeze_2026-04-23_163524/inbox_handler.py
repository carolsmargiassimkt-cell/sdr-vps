...
def update_crm_status_for_lead(lead_state, status_bot):
    ...
    for deal in list(related_deals_from_state(lead_state) or []):
        ...
        if deal_id <= 0:
            continue
        try:
            ok = None if is_test_whitelist_phone(phone) else crm.update_deal(deal_id, {crm.STATUS_BOT_FIELD: str(status_bot or "").strip()})
        except Exception:
            ok = False
        updated = updated or bool(ok)
    if current_person_id > 0:
        try:
            None if is_test_whitelist_phone(phone) else crm.update_person(current_person_id, {crm.STATUS_BOT_FIELD: str(status_bot or "").strip()})
        except Exception:
            pass
    return updated
    ...
    try:
        ...
        if is_hard_negative_message(message):
            try:
                ...
                pass
            print(f"[BLOCKLIST_ADICIONADO] telefone=55{phone} motivo=sem_interesse_hard_rule")
            print(f"[INTENT_NEGATIVO_HARD_RULE] telefone={phone}")
            lead_state["status_bot"] = "sem_interesse"
            try:
                update_crm_status_for_lead(lead_state, "sem_interesse")
                append_crm_note_for_lead(lead_state, "Lead informou nao ter interesse. Movido para blocklist por hard rule.")
            except Exception:
                pass
            ...
            if ok:
                append_history(phone, "out", reply, step=0)
                print(f"[RESPOSTA_ENVIADA] {phone}")
            return {"ok": True, "status": "sem_interesse_hard_rule"}
        ...
        if has_recent_history_entry(phone, "in", message, within_seconds=900) and not is_test_whitelist_phone(phone):
